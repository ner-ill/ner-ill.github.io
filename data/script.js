(() => {
  ...
  const storyContent = esc(game.story).split('\n\n').map(paragraph => `<p>${paragraph}</p>`).join('');
  ...
  const detailLink = (title, description, url) => `<section class="detail-entry"><h2>${url ? `<a href="${esc(url)}">${title}<span aria-hidden="true">→</span></a>` : title}</h2><p>${description}</p>${url ? '' : '<p class="empty">準備中</p>'}</section>`;
  const resourceEntry = (title, description, url, label) => `<section class="resource-entry"><h2>${title}</h2><p>${description}</p>${url ? `<a class="resource-link" href="${esc(url)}" target="_blank" rel="noopener">${label} <span aria-hidden="true">↗</span></a>` : '<p class="empty">準備中</p>'}</section>`;
  page.innerHTML=`<article class="game-content game-intro"><p class="eyebrow">GAME</p><h1>${esc(game.title)}</h1><p class="catch">${esc(game.catchcopy)}</p></article><div class="game-hero"><img src="${image(game.keyVisual)}" alt="${esc(game.title)}のキービジュアル"></div><article class="game-content game-sections"><section><h2>Story</h2>${storyContent}</section><section><h2>Information</h2><dl class="info"><dt>ジャンル</dt><dd>${esc(game.genre)}</dd><dt>公開日</dt><dd>${esc(game.releaseDate)}</dd><dt>対応OS</dt><dd>${esc(game.platform)}</dd><dt>価格</dt><dd>${esc(game.price)}</dd></dl></section>${detailLink('Character', '登場人物を見る', game.characterUrl)}${detailLink('Gallery', 'ギャラリーを見る', game.galleryUrl)}<section><h2>Download</h2>${downloadContent}</section>${resourceEntry('Novel', '小説・後日談・IF・短編など', game.novelUrl, 'noteの作品別マガジンを読む')}${resourceEntry('Materials', '設定資料・ラフ・未公開イラストなど', game.materialsUrl, '設定資料を見る')}${resourceEntry('18+', '成人向け作品への外部リンク', game.adultUrl, '18歳以上向け作品はこちら')}<a class="back" href="index.html">← Game一覧へ戻る</a></article>`;
  } }
})();
